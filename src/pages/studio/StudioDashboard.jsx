    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiTile icon={MessageCircle} label="Applicants"      value={kpis.applicants_total ?? 0}
          sub={`${kpis.applicants_this_week ?? 0} this week`} color="#B4FF5A" loading={loading} />
        <KpiTile icon={Heart}         label="Saved"           value={kpis.saved_instructors ?? 0}
          sub="Instructors in favourites"                     color="#B4FF5A" loading={loading} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          { to: '/studio/search',   icon: Search,    color: '#B4FF5A', title: 'Find Instructors',  desc: 'Browse our active community' },
          { to: '/studio/profile',  icon: Building2, color: '#B4FF5A', title: 'Studio Profile',    desc: 'Update your studio details' },
        ].map((q) => (
          ...
          renderItem={(inst) => (
            <div className="flex items-center gap-3">
              ...
              <Button variant="ghost" size="xs" iconRight={ArrowRight}
                ...
                className="!text-coral hover:!underline">View</Button>
            </div>
          )}
        />
      </div>
