...
export default function SearchInstructors() {
  ...
  const allDisciplines = useMemo(() => {
    return [...new Set(instructors.flatMap(i => i.disciplines || []))].sort();
  }, [instructors]);
                ...
                <div className="px-5 pb-4 space-y-3">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-1.5 text-xs text-[#6B6B66]">
                      ...
                      <span className="text-[#9A9A94]">From</span> {countryFrom} {travelingTo && <>→ <span className="font-medium">{travelingTo}</span></>}
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-[#6B6B66]">
